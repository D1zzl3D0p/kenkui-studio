  ...
  async function play(id: string, url: string, fraction = 0) {
    ...
    element.onerror = () => {
      if (audio.current === element) {
        stop();
        setError("This sample could not load. Please try again.");
      }
    };
    if (fraction)
    ...
    try {
      ...
      /* A static waveform is preferable to losing the audio. */
    }
    if (ticket !== sequence.current) return;
    try {
      await element.play();
...
export function Wave({
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      if (!active || motion.matches || !node) {
        rest();
        return;
      }
      const data = new Uint8Array(node.frequencyBinCount);
      const tick = () => {
        if (analyser.current !== node) {
          rest();
          return;
        }
        ...
        bars.forEach((bar, i) => {
          ...
          const start =
            1 + Math.floor((i * (data.length / 2 - 1)) / bars.length);
          const amplitude =
            (data[start] + data[Math.min(start + 1, data.length - 1)]) / 510;
          bar.style.transform = `scaleY(${Math.max(0.1, amplitude)})`;
        });
  ...
  return (
    <span
      ref={root}
      className={`wave ${active ? "speaking" : ""}`}
      aria-hidden="true"
    >
      {Array.from({ length: 26 }, (_, i) => (
        <i key={i} style={{ height: 30, transform: "scaleY(.16)" }} />
      ))}
    </span>
  );
}
